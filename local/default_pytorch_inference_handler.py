...
# from https://github.com/aws/sagemaker-pytorch-inference-toolkit/blob/master/src/sagemaker_pytorch_serving_container/default_pytorch_inference_handler.py
# mplement an inference handler, which is responsible for loading the model and providing input, predict, and output functions.
...
# Note, passing context as an argument to the handler functions is optional. 
# Customer can choose to omit context from the function declaration if it's not needed in the runtime. 
# For example, the following handler function declarations will also work:
#   def default_model_fn(self, model_dir)
